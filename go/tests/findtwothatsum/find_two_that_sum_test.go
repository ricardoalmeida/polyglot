...
// https://github.com/joncalhoun/algorithmswithgo.com/tree/master/module01#12---find-two-that-sum-code
...
func TestFindTwoThatSum(t *testing.T) {
	for _, tc := range TESTS {
		t.Run(fmt.Sprintf("%v with sum %v", tc.numbers, tc.sum), func(t *testing.T) {
			...
			i, j := findtwothatsum.FindTwoThatSum(numbersCopy, tc.sum)
