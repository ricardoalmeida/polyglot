use polyglot::{base_to_dec::iterate, base_to_dec::std};

#[test]
fn test_iterate() {
    for (want, base, have) in TESTS {
        assert_eq!(iterate(have.to_string(), *base), *want);
    }
}
...
fn test_std() {
    for (want, base, have) in TESTS {
        assert_eq!(std(have.to_string(), *base), *want);
    }
}
